...
// Function to handle application post
export const postApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!resume) {
    return next(new ErrorHandler("Resume file not found in the request!", 400));
  }

  // Allow only image and PDF file types
  const allowedFormats = ["image/png", "image/jpeg", "image/webp", "application/pdf"];
  if (!allowedFormats.includes(resume.mimetype)) {
    return next(new ErrorHandler("Invalid file type. Please upload a valid format (PNG, JPEG, WEBP, or PDF).", 400));
  }
